 ...
 * <p>
 * Supports lossless delta- and key-frame encoding of images with 8, 16 or
 * 24 bits per pixel.
 * <p>
...
public class AnimationCodec extends AbstractVideoCodec {
  ...
  public Format setInputFormat(Format f) {
    if (f instanceof VideoFormat) {
      ...
      if (BufferedImage.class.isAssignableFrom(vf.getDataClass())) {
        return super.setInputFormat(new VideoFormat(VideoFormat.QT_ANIMATION,VideoFormat.QT_ANIMATION_COMPRESSOR_NAME,vf.getDataClass(), vf.getWidth(), vf.getHeight(), depth));
      }
    }
  ...
  public Format setOutputFormat(Format f) {
    if (f instanceof VideoFormat) {
      ...
      return super.setOutputFormat(new VideoFormat(VideoFormat.QT_ANIMATION,VideoFormat.QT_ANIMATION_COMPRESSOR_NAME,byte[].class, vf.getWidth(), vf.getHeight(), depth));
    }
      ...
      switch (vf.getDepth()) {
        ...
        case 16: {
          ...
          if ((in.flags & Buffer.FLAG_KEY_FRAME) != 0 //
            ...
            /*
            if (testcast == null) {
              testcast = pixels.clone();
            } else {
              System.arraycopy(pixels, 0, testcast, 0, pixels.length);
            }
            decodeDelta16(new ByteArrayImageOutputStream(tmp.getBuffer(), 0, (int) tmp.getStreamPosition(), ByteOrder.BIG_ENDIAN),//
                    testcast, (short[]) previousPixels, r.width, r.height, r.x + r.y * scanlineStride, scanlineStride);
             */
          }
          if (previousPixels == null) {
            previousPixels = pixels.clone();
